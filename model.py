...
class CSRNet(nn.Module):
    def __init__(self, load_weights=False):
        ...
        self.backend_feat  = [256,256,128,128,64,64] #,256,128,64]
        self.backend_feat2 = [64]
        self.frontend = make_layers(self.frontend_feat)
        self.backend = make_layers(self.backend_feat,in_channels = 512,dilation = False)
        self.backend_res = make_res_layer(512,64)
        self.backend_up_conv = make_conv(128,64,1)
        self.backend_up_conv1 = make_layers(self.backend_feat2,in_channels = 64,dilation = False)
...
def make_output(in_channel):
    ...
    #layers =[nn.Conv2d(64, 1, kernel_size=1), nn.ReLU(inplace=True)]
    return nn.Sequential(*layers)              
